...
import {
  getAccount, getPositions, getPortfolioHistory, getPendingTrades,
  getSignals, getNews, approveTrade, rejectTrade, getQuotes,
  getActivityFeed, liquidateAll, getWatchlist, type NewsGroup,
} from "@/lib/api";
...
function ActivityFeedInline() {
  const { data: events } = useQuery({ queryKey: ["activity"], queryFn: getActivityFeed, refetchInterval: 5000 });
...
function Pulse() {
  return <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "#22c55e", animation: "pulse 2s ease-in-out infinite", marginRight: 6 }} />;
}
...
function Sparkline({ values }: { values: number[] }) {
  if (!values.length) return <div style={{ height: 40, display: "flex", alignItems: "center", justifyContent: "center", color: "#4a4a6a", fontSize: 11 }}>accumulating data...</div>;
...
function TickerBar({ quotes, tickers }: { quotes: Record<string, number>; tickers: string[] }) {
  return (
  ...
  return (
    <div style={{ padding: "5px 20px", flexShrink: 0, background: status.isOpen ? "#0a1a0a" : "#111108", borderBottom: "1px solid #1e1e2e", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 11 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ color: "#4a4a6a" }}>{status.isOpen ? "orders execute immediately" : "approved trades queue — execute at 9:30 PM SGT open"}</span>
      </div>
...
const PANEL: React.CSSProperties = { background: "#0d0d14", padding: 14, display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 };
const LABEL: React.CSSProperties = { fontSize: 10, letterSpacing: "0.1em", color: "#4a4a6a", textTransform: "uppercase", marginBottom: 10, display: "flex", alignItems: "center", gap: 6, flexShrink: 0 };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const tick = () => setNow(new Date().toLocaleString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit", timeZoneName: "short" }));
    tick();
  ...
  const { mutate: approve }        = useMutation({ mutationFn: approveTrade,  onSuccess: () => qc.invalidateQueries(), onError: () => qc.invalidateQueries() });
  const { mutate: reject }         = useMutation({ mutationFn: rejectTrade,   onSuccess: () => qc.invalidateQueries() });
  const { mutate: doLiquidateAll } = useMutation({ mutationFn: liquidateAll,  onSuccess: () => qc.invalidateQueries() });

  const totalPL  = positions?.reduce((s, p) => s + p.unrealized_pl, 0) ?? 0;
  const equity   = account?.portfolio_value ?? 0;
  const tickerList = watchlist?.map(w => w.ticker) ?? ["SPY","QQQ","XLE","GLD","TLT","XLK","XLF","XLI","XLV","SLV","USO","BTC/USD"];
  ...
  return (
    <>
      ...
      <div suppressHydrationWarning style={{ background: "#0a0a0f", color: "#e2e2e8", fontFamily: "'SF Mono','Fira Code','Cascadia Code',monospace", fontSize: 13, borderRadius: 10, overflow: "hidden", border: "1px solid #1e1e2e", width: "100%", height: "calc(100vh - 48px)", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ background: "#0d0d14", padding: "10px 20px", borderBottom: "1px solid #1e1e2e", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
            <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#4a4a6a" }}><Pulse />live · paper mode</span>
            {now && <span style={{ fontSize: 11, color: "#4a4a6a" }}>{now}</span>}
          </div>
        </div>

        <TickerBar quotes={quotes ?? {}} tickers={tickerList} />
        <MarketStatusBar />

        {/* Body grid */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gridTemplateRows: "1fr 160px", gap: 1, background: "#1e1e2e", flex: 1, minHeight: 0, overflow: "hidden" }}>
          ...
          <div style={PANEL}>
            <div style={{ ...LABEL, justifyContent: "space-between" }}>
              <span>portfolio</span>
              {!!positions?.length && (
                <button onClick={() => { if (window.confirm("Liquidate all positions?")) doLiquidateAll(); }}
                ...
                <div key={p.ticker} className="dash-row" style={{ ...DIVIDER, padding: "7px 4px", transition: "background 0.15s", background: (p as any).pct_to_stop < 1.0 ? "rgba(239,68,68,0.04)" : undefined }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      {(p as any).pct_to_stop < 1.0 && <span style={{ fontSize: 9, background: "#2e0d0d", color: "#ef4444", border: "1px solid #4a1a1a", padding: "1px 4px", borderRadius: 3 }}>⚠ SL</span>}
                    </div>
                  ...
                  {(p as any).stop_loss && (
                    <div style={{ marginTop: 5, display: "flex", alignItems: "center", gap: 5 }}>
                      ...
                      <div style={{ flex: 1, height: 2, background: "#1e1e2e", borderRadius: 1, position: "relative", overflow: "hidden" }}>
                        <div style={{ position: "absolute", left: 0, top: 0, height: "100%", borderRadius: 1, width: `${Math.min(Math.max(((p.current_price - (p as any).stop_loss) / ((p as any).take_profit - (p as any).stop_loss)) * 100, 0), 100)}%`, background: (p as any).pct_to_stop < 1.0 ? "#ef4444" : (p as any).pct_to_target < 1.0 ? "#22c55e" : "#f59e0b" }} />
                      </div>
            ...
            <div className="dash-scroll" style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
              {signals?.slice(0, 12).map((s) => (
                <div key={s.id} className="dash-row" style={{ ...DIVIDER, display: "flex", alignItems: "center", gap: 8, padding: "7px 4px", transition: "background 0.15s" }}>
                  ...
                  <span style={{ fontWeight: 600, color: "#a0a0c0", minWidth: 50, fontSize: 11 }}>{s.ticker}</span>
                  <span style={{ color: "#4a4a6a", fontSize: 10, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.reasoning}</span>
                  <span style={{ fontSize: 10, fontWeight: 600, color: ConfColor(s.confidence), minWidth: 28, textAlign: "right", flexShrink: 0 }}>{(s.confidence * 100).toFixed(0)}%</span>
                </div>
              ))}
          ...
          <div style={PANEL}>
            ...
            <div className="dash-scroll" style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
                  ...
                  <div key={t.id} style={{ marginBottom: 8, borderRadius: 6, overflow: "hidden", border: `1px solid ${isExit ? "#4a1a1a" : "#1e1e2e"}`, background: isExit ? "rgba(239,68,68,0.04)" : "#111118" }}>
                    <div style={{ padding: "8px 10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <div style={{ textAlign: "right" }}>
                        ...
                        {(t as any).stop_loss && <div style={{ fontSize: 9, color: "#4a4a6a", marginTop: 1 }}>SL {fmt$$((t as any).stop_loss)} · TP {fmt$$((t as any).take_profit)}</div>}
                      </div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderTop: "1px solid #1e1e2e", gap: 1, background: "#1e1e2e" }}>
                      <button className="approve-btn" onClick={() => reject(t.id)} style={{ background: "#0d0d14", color: "#6a6a8a" }}
                      ...
                      <button className="approve-btn" onClick={() => approve(t.id)} style={{ background: isExit ? "#2e0d0d" : "#0d2e1a", color: isExit ? "#ef4444" : "#22c55e" }}
              ...
              {!pending?.length && <div style={{ padding: "16px 8px", textAlign: "center", color: "#4a4a6a", fontSize: 11, border: "1px solid #1e1e2e", borderRadius: 6 }}>✓ no pending trades</div>}
            </div>
            <div style={{ flexShrink: 0, marginTop: 12, paddingTop: 12, borderTop: "1px solid #1e1e2e" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 600, color: totalPL >= 0 ? "#22c55e" : "#ef4444" }}>{totalPL >= 0 ? "+" : ""}{fmt$$(totalPL)}</span>
            ...
            <div style={{ padding: "10px 14px", display: "flex", flexDirection: "column", width: "100%" }}>
              ...
              <div className="dash-scroll" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0 16px", overflowY: "auto", flex: 1, minHeight: 0, alignContent: "start" }}>
                {newsFlat.slice(0, 6).map((a, i) => (
                  <div key={`${a.url}-${i}`} style={{ paddingBottom: 8, cursor: "pointer" }} onClick={() => a.url && window.open(a.url, "_blank")}>
                    <div style={{ marginBottom: 3, display: "flex", alignItems: "center", gap: 4 }}>
                      <span style={{ ...(srcStyle[a.source] ?? { background: "#1a1a2e", color: "#6a6a8a", border: "1px solid #2a2a4a" }), fontSize: 9, letterSpacing: "0.06em", padding: "1px 5px", borderRadius: 2, display: "inline-block" }}>
                        {a.source}{a.tickers?.length ? ` · ${a.tickers[0]}` : ""}
                      </span>
                      {(a as any).signal_class && (a as any).signal_class !== "NOISE" && (a as any).signal_class !== "MONITORING" && (
                        <span style={{
                          fontSize: 8, padding: "1px 4px", borderRadius: 2,
                          ...((a as any).signal_class === "CRITICAL" ? { background: "#2e0d0d", color: "#ef4444", border: "1px solid #4a1a1a" } : { background: "#2e1a0d", color: "#f97316", border: "1px solid #5a3a1a" }),
                        }}>
                          {(a as any).signal_class}
                        </span>
                      )}
                    </div>
