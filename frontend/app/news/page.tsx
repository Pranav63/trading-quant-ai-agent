...
const srcLabel: Record<string, string> = {
  ...
  rss_wsj_markets: "WSJ", rss_ft_markets: "FT",
  "rss_investing.com_economy": "investing.com",
}

const WATCHLIST = ["SPY", "QQQ", "XLE", "GLD", "TLT", "XLK", "XLF", "XLI", "XLV"]
const ALL_SOURCES = ["ALL", "finnhub", "newsapi", "fred", "rss_reuters_business",
  "rss_reuters_markets", "rss_wsj_markets", "rss_ft_markets", "rss_investing.com_economy"]

export default function NewsPage() {
  const { data: articles, isLoading, dataUpdatedAt } = useQuery({
    queryKey: ["news_flat"],
    queryFn: getNewsFlat,
  ...
  const filtered = articles?.filter(a => {
    ...
    const matchTicker = tickerFilter === "ALL"
  ...
  return (
    <div style={{
      fontFamily: "var(--dash-mono)", display: "flex", flexDirection: "column", gap: 14,
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: "#4a4a6a", marginTop: 2 }}>
              {filtered?.length ?? 0} articles · sources: finnhub, newsapi, reuters, WSJ, FT, FRED
      ...
      <div className="scroll-hide" style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
        {isLoading && <div style={{ padding: "40px 0", textAlign: "center", color: "#4a4a6a", fontSize: 12 }}>loading articles...</div>}

        {!isLoading && layout === "grid" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, alignContent: "start" }}>
            {filtered?.map(a => (
              <div key={a.id} className="news-card" onClick={() => a.url && window.open(a.url, "_blank")}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  {srcBadge(a.source, a.tickers)}
                  {a.tickers?.map(t => (
                    <span key={t} style={{ background: "#13133a", color: "#7c7cdc", border: "1px solid #2a2a6a", fontSize: 9, padding: "1px 5px", borderRadius: 3, fontWeight: 600 }}>{t}</span>
                  ))}
                  {a.sentiment_raw != null && (
                    ...
                    }}>{(a.sentiment_raw * 100).toFixed(0)}% bull</span>
                  )}
                  <span style={{ marginLeft: "auto", fontSize: 9, color: "#4a4a6a" }}>↗</span>
                </div>
                <div style={{ fontSize: 13, color: "#c0c0d8", lineHeight: 1.5, fontWeight: 500 }}>{a.headline}</div>
                {a.summary && (
                  <div style={{ fontSize: 11, color: "#6a6a8a", lineHeight: 1.4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                    {a.summary}
                  </div>
                )}
          ...
          <div style={{ background: "#0d0d14", border: "1px solid #1e1e2e", borderRadius: 8, overflow: "hidden" }}>
            {filtered?.map((a, i) => (
              <div key={a.id} className="news-card-list" onClick={() => a.url && window.open(a.url, "_blank")}
                style={{ borderBottom: i === (filtered.length - 1) ? "none" : "1px solid #1e1e2e" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, color: "#c0c0d8", fontWeight: 500, lineHeight: 1.4, marginBottom: 4 }}>{a.headline}</div>
                  {a.summary && (
                    <div style={{ fontSize: 11, color: "#6a6a8a", lineHeight: 1.35, display: "-webkit-box", WebkitLineClamp: 1, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                      {a.summary}
                    </div>
                  )}
                </div>
        ...
        {!isLoading && !filtered?.length && (
          <div style={{ padding: "40px 0", textAlign: "center", color: "#4a4a6a", fontSize: 12 }}>
            {search || sourceFilter !== "ALL" || tickerFilter !== "ALL" ? "no articles match your filters" : "no articles yet — run ingestion"}
          </div>
        )}
      </div>
    </div>
