...
function SentimentBadge({ value }: { value: number }) {
  ...
  return (
    <span style={{ ...s, fontSize: 8, padding: "1px 5px", borderRadius: 3 }}>
      {label}
    </span>
  );
}
...
function CrossSourcePanel() {
  const { data, isLoading } = useQuery({
    queryKey: ["news_clusters"],
    queryFn: () => getNewsClusters(6),
    ...
    >
      ...
      >
        ...
        {data?.clusters.map((cluster, i) => {
          const sc =
            SIGNAL_COLOR[cluster.signal_class] ?? SIGNAL_COLOR.MONITORING;
          return (
            <div
              key={i}
              style={{
                background: "#0d0d14",
                border: `1px solid ${cluster.signal_class === "CRITICAL" ? "#3a1a1a" : "#1e1e2e"}`,
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                ...
                <span
                  style={{ marginLeft: "auto", fontSize: 8, color: "#3a3a5a" }}
                >
                  {cluster.source_count} sources · {cluster.article_count}
                </span>
              </div>
              ...
              <div style={{ fontSize: 10, color: "#c0c0d8", lineHeight: 1.4 }}>
                {cluster.articles[0]?.headline}
              </div>
              ...
              <div style={{ fontSize: 8, color: "#3a3a5a" }}>
                {fmtTime(cluster.latest_at)}
              </div>
            </div>
          );
        })}
        {!isLoading && !data?.clusters.length && (
          <div
            style={{
              fontSize: 10,
              color: "#4a4a6a",
...
export default function NewsPage() {
  ...
  const filtered = articles.filter((a) => {
    ...
    const matchSignal =
      signalFilter === "ALL" ||
      (a.signal_class ?? "MONITORING") === signalFilter;
    const matchSearch =
      !search ||
  ...
  const signalCounts = SIGNAL_CLASSES.reduce(
    (acc, cls) => {
      acc[cls] =
        cls === "ALL"
          ? articles.length
          : articles.filter((a) => (a.signal_class ?? "MONITORING") === cls)
              .length;
      return acc;
    },
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 10, color: "#3a3a5a", marginTop: 2 }}>
              {filtered.length} articles · finnhub, newsapi, reuters, WSJ, FT,
              FRED
              {lastUpdated && (
                <span style={{ marginLeft: 6 }}>· updated {lastUpdated}</span>
              )}
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
            ...
            >
              {SIGNAL_CLASSES.map((cls) => (
                ...
                >
                  {cls}
                  {signalCounts[cls] > 0 ? ` (${signalCounts[cls]})` : ""}
                </button>
              ))}
            </div>
          </div>
                ...
                >
                  ...
                  >
                    ...
                    {a.sentiment_raw != null && (
                      <SentimentBadge value={a.sentiment_raw} />
                    )}
                    <span
                      style={{
                        marginLeft: "auto",
                  ...
                  >
                    ...
                    {a.url && (
                      <span style={{ fontSize: 9, color: "#2a2a6a" }}>
                        click to open →
                      </span>
                    )}
                  </div>
              ...
              {filtered.map((a, i) => (
                <div
                  ...
                  style={{
                    ...
                    borderLeft:
                      a.signal_class === "CRITICAL"
                        ? "2px solid #ef4444"
                        : a.signal_class === "ELEVATED"
                          ? "2px solid #f97316"
                          : "none",
                  }}
                >
