...
export default function NewsPage() {
  ...
  const sources = ["ALL", "finnhub", "newsapi", "reddit"]
  const tickers = ["ALL", ...WATCHLIST]

  const filtered = articles?.filter(a => {
    const matchSource = sourceFilter === "ALL" || a.source === sourceFilter
    const matchTicker = tickerFilter === "ALL" || a.tickers?.includes(tickerFilter)
    ...
    <div style={{
      ...
      <style>{`
        ...
        .filter-pill {
          ...
          font-family: inherit; border: 1px solid #1e1e2e;
        ...
        .ticker-pill:hover:not(.active) { border-color: #2a2a4a; color: #a0a0c0; }
        .layout-btn {
          padding: 4px 8px; border-radius: 4px; font-size: 11px;
          cursor: pointer; transition: all 0.15s; font-family: inherit;
          border: 1px solid #1e1e2e; background: #0d0d14; color: #4a4a6a;
        }
        ...
        .search-input {
          ...
          padding: 6px 12px; font-size: 11px; color: #e2e2e8;
      ...
      <div style={{ flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: "#4a4a6a", marginTop: 2 }}>
              {filtered?.length ?? 0} of {articles?.length ?? 0} articles
              {lastUpdated && <span style={{ marginLeft: 8 }}>· last updated {lastUpdated}</span>}
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 8, flexWrap: "wrap" }}>
          <span style={{ fontSize: 10, color: "#4a4a6a", marginRight: 2 }}>source:</span>
          {sources.map(s => (
            <button
              key={s}
          ...
          <div style={{ width: 1, height: 14, background: "#1e1e2e", margin: "0 4px" }} />
          <span style={{ fontSize: 10, color: "#4a4a6a", marginRight: 2 }}>ticker:</span>
            ...
            {filtered?.map(a => (
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  ...
                  <span style={{ marginLeft: "auto", fontSize: 9, color: "#4a4a6a", flexShrink: 0 }}>
                    ↗
                  </span>
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 2 }}>
                  ...
                  {a.url && (
                    <span style={{ fontSize: 10, color: "#2a2a6a" }}>click to open →</span>
                  )}
                </div>
              </div>
            ))}
        ...
        {!isLoading && layout === "list" && (
          <div style={{
            ...
            {filtered?.map((a, i) => (
              ...
              >
                {/* Left: source + sentiment */}
                <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", gap: 6, minWidth: 80 }}>
                  <span style={{
                    ...srcStyle[a.source] ?? srcStyle.newsapi,
                    fontSize: 9, letterSpacing: "0.06em",
                ...
                <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, minWidth: 80 }}>
                  ...
                  <span style={{ fontSize: 10, color: "#4a4a6a", whiteSpace: "nowrap" }}>
