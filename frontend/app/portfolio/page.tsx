...
export default function PortfolioPage() {
  ...
  const totalVal =
    positions?.reduce((s, p) => s + p.qty * p.current_price, 0) ?? 0;
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 11, color: "#4a4a6a", marginTop: 2 }}>
            positions · stop loss · take profit · liquidation controls
          </div>
        </div>
        {!!positions?.length && (
          <button
          ...
          >
            {liquidatingAll ? "closing all..." : "⚡ liquidate all positions"}
          </button>
        )}
      </div>
      ...
      <div className="dash-card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        >
          ...
          <span style={{ fontSize: 10, color: "#4a4a6a" }}>
            SL = entry ∓ (ATR × 2) · TP = entry ∓ (ATR × 3) · direction-aware
          </span>
        </div>
        <table
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    <span
                      style={{
                        ...
                        borderRadius: 3,
                        fontWeight: 600,
                        background: side === "long" ? "#0d2e1a" : "#2e0d0d",
                        color: side === "long" ? "#22c55e" : "#ef4444",
                        border: `1px solid ${side === "long" ? "#1a4a2a" : "#4a1a1a"}`,
                      }}
                    >
                      {side}
                    </span>
                  ...
                  <td style={{ padding: "12px 14px", textAlign: "right" }}>
                    <button
                      onClick={() => {
                        if (window.confirm(`Liquidate ${p.ticker}?`))
                          doLiquidate(p.ticker);
                      }}
                      disabled={isLiq}
                      style={{
                        padding: "4px 10px",
                        fontSize: 10,
                        fontWeight: 600,
                        cursor: isLiq ? "not-allowed" : "pointer",
                        fontFamily: "inherit",
                        background: "#1a0d2e",
                        color: "#a855f7",
                      ...
                      onMouseEnter={(e) => {
                        if (!isLiq)
                          e.currentTarget.style.background = "#2e1a4a";
                      }}
                      onMouseLeave={(e) => {
      ...
      >
        <span>
          range: <span style={{ color: "#ef4444" }}>red = near SL</span> ·{" "}
          <span style={{ color: "#f59e0b" }}>amber = mid</span> ·{" "}
          <span style={{ color: "#22c55e" }}>green = near TP</span>
        </span>
        <span>·</span>
        <span>SL/TP direction-aware — shorts invert automatically</span>
        <span>·</span>
        <span>
          position monitor checks every 5 min · news exit triggers above 70%
          confidence
        </span>
      </div>
    </div>
