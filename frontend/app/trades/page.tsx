...
function RiskMeter({ pct }: { pct: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontSize: 10, color, fontWeight: 600, minWidth: 60 }}>
        {label}
      </span>
    </div>
  );
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 20, fontWeight: 700, color: "#e2e2e8" }}>
            {trade.ticker}
          </span>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 20, fontWeight: 700, color: "#e2e2e8" }}>
            {fmt$$(trade.notional)}
          </div>
          ...
          <div>
            ...
            <div
              style={{
                ...
                fontWeight: 600,
                color:
                  trade.rr_ratio && trade.rr_ratio >= 1.5
                    ? "#22c55e"
                    : "#f59e0b",
              }}
            >
          ...
          <div>
            ...
            <div
              style={{
                ...
                fontWeight: 600,
                color:
                  (trade.risk_pct_of_account ?? 0) < 1
                    ? "#22c55e"
                    : (trade.risk_pct_of_account ?? 0) < 2
                      ? "#f59e0b"
                      : "#ef4444",
              }}
            >
              {trade.risk_pct_of_account != null
                ? `${trade.risk_pct_of_account}%`
                : "—"}
            </div>
            <div style={{ fontSize: 10, color: "#4a4a6a", marginTop: 2 }}>
              of portfolio
      ...
      {hasRisk && (
        <div
          style={{ padding: "10px 16px", borderBottom: "1px solid #1e1e2e" }}
        >
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <span style={{ fontSize: 9, color: "#7c7c9a" }}>
              entry {fmt$$(trade.current_price!)}
            </span>
            <span style={{ fontSize: 9, color: "#22c55e" }}>
              TP {fmt$$(trade.take_profit!)}
            </span>
          </div>
          <div
            style={{
      ...
      >
        <div>
          ...
          <span style={{ fontSize: 11, color: "#a0a0c0" }}>
            {isExit ? "position monitor" : "llm signal"}
          </span>
        </div>
        <div>
          <span
        ...
        <div>
          ...
          <span style={{ fontSize: 11, color: "#a0a0c0" }}>
            {trade.shares != null ? trade.shares : "—"}
          </span>
        </div>
      </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: isExecuted ? "1fr 1fr 1fr" : "1fr 1fr",
          gap: 1,
          background: "#1e1e2e",
        }}
      >
        ...
        {isExecuted && (
          <button
            ...
            style={{
              ...
              border: "none",
              transition: "all 0.15s",
              opacity: liquidating ? 0.5 : 1,
            }}
            onMouseEnter={(e) => {
              if (!liquidating) e.currentTarget.style.background = "#2e1a4a";
            }}
            onMouseLeave={(e) => {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 11,
              fontWeight: 600,
              cursor: liquidatingAll ? "not-allowed" : "pointer",
              fontFamily: "inherit",
              letterSpacing: "0.04em",
              background: "#2e0d0d",
              color: "#ef4444",
              border: "1px solid #4a1a1a",
              borderRadius: 6,
              opacity: liquidatingAll ? 0.5 : 1,
              transition: "all 0.15s",
            }}
            onMouseEnter={(e) => {
              if (!liquidatingAll) e.currentTarget.style.background = "#4a1a1a";
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.background = "#2e0d0d";
            }}
          >
            {liquidatingAll ? "closing..." : "⚡ liquidate all"}
          </button>
        </div>
      </div>
      ...
      >
        ...
        {sells.length > 0 && (
          <div style={{ flexShrink: 0 }}>
            ...
            >
              ⚠ exit signals — stop loss / take profit / news triggered
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {sells.map((t) => (
                <PendingCard
                  ...
                  onApprove={approve}
                  onReject={reject}
                  onLiquidate={doLiquidate}
                  approving={approvingId === t.id}
                  rejecting={rejectingId === t.id}
                  liquidating={liquidatingId === t.id}
