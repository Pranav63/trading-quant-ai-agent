...
function MacroTile({ ind }: { ind: MacroIndicator }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: 8, color: "#3a3a5a", letterSpacing: "0.08em", fontWeight: 600 }}>
        {ind.label.toUpperCase()}
        {ind.id && <span style={{ color: "#2a2a4a", marginLeft: 3 }}>{ind.id}</span>}
      </span>
      <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: changeColor === "#4a4a6a" ? "#c0c0d8" : changeColor }}>
          {ind.unit === "$" ? `$${fmt(ind.value)}` : `${fmt(ind.value)}${ind.unit}`}
        </span>
...
export default function MacroStrip() {
  ...
  return (
    ...
    >
      <span style={{
        fontSize: 8, fontWeight: 700, color: "#3a3a5a",
        letterSpacing: "0.1em", padding: "0 12px",
        borderRight: "1px solid #1a1a2a", flexShrink: 0,
        whiteSpace: "nowrap",
      }}>
        MACRO
      </span>
      ...
      {data.indicators[0]?.date && (
        <span style={{ fontSize: 8, color: "#2a2a4a", padding: "0 12px", flexShrink: 0, marginLeft: "auto" }}>
