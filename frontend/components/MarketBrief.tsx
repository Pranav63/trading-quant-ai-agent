...
export default function MarketBrief() {
  ...
  useEffect(() => {
    ...
    const tick = () => setCountdown(Math.ceil(Math.max(0, 15 * 60 * 1000 - (Date.now() - dataUpdatedAt)) / 60000));
    tick();
  ...
  const sc = SIGNAL_COLOR[theme] ?? { color: "#7c7cdc", bg: "#13133a", border: "#2a2a6a" };
  const isQuiet = !data?.brief || data.article_count === 0 || theme === "pending" || theme === "quiet";
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        <span style={{ fontSize: 9, fontWeight: 700, color: "#4a4a6a", letterSpacing: "0.1em" }}>
          AI MARKET BRIEF
        </span>

        {!isQuiet && (
          <span style={{ background: sc.bg, color: sc.color, border: `1px solid ${sc.border}`, fontSize: 8, padding: "1px 6px", borderRadius: 3, letterSpacing: "0.05em" }}>
            {theme.toUpperCase()}
          </span>
        )}

        {data?.affected_etfs?.map(etf => (
          <span key={etf} style={{ background: "#13133a", color: "#7c7cdc", border: "1px solid #2a2a6a", fontSize: 8, padding: "1px 4px", borderRadius: 3, fontWeight: 700 }}>
            {etf}
          </span>
        ))}

        {!isQuiet && data?.article_count != null && (
          <span style={{ fontSize: 8, color: "#2a2a4a" }}>{data.article_count} signals</span>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 1 }}>
            {generatedAt && (
              <span style={{ fontSize: 8, color: "#2a2a4a" }}>
                {generatedAt}{data?.provider ? ` · ${data.provider}` : ""}
              </span>
            )}
            {countdown !== null && countdown > 0 && !isQuiet && (
              <span style={{ fontSize: 8, color: "#1e1e3a" }}>refreshes in {countdown}m</span>
            )}
          </div>

          <button className="brief-btn" onClick={() => generate()} disabled={generating}>
            <span className={spinning ? "spin" : ""} style={{ fontSize: 10, lineHeight: 1 }}>↻</span>
      ...
      {isLoading ? (
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span className="spin" style={{ fontSize: 12, color: "#4a4a6a" }}>↻</span>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div style={{ fontSize: 11, color: "#3a3a5a", lineHeight: 1.6 }}>
            ...
            <span style={{ color: "#2a2a4a" }}>
              Run ingestion from the sidebar to fetch latest news, then hit refresh.
            </span>
          </div>
          <button className="brief-btn" onClick={() => generate()} disabled={generating} style={{ flexShrink: 0, borderColor: "#2a2a6a", color: "#7c7cdc" }}>
            <span className={spinning ? "spin" : ""} style={{ fontSize: 10 }}>↻</span>
            refresh
          </button>
        </div>
      ) : (
