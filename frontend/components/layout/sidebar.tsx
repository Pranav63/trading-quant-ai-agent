...
export function Sidebar() {
  ...
  return (
    <aside style={{
      ...
      display: "flex", flexDirection: "column", fontFamily: "var(--dash-mono)",
    }}>
      <style>{`
      ...
      <div style={{ padding: "20px 16px 16px", borderBottom: "1px solid #1e1e2e" }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: "#7c7cdc", letterSpacing: "0.04em" }}>
          ⬡ trading agent
        </div>
        <div style={{ fontSize: 10, color: "#4a4a6a", marginTop: 4, letterSpacing: "0.08em" }}>
          PAPER MODE · ALPACA
        </div>
      </div>
      ...
      <div style={{ padding: "12px 10px", borderTop: "1px solid #1e1e2e", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button
            ...
            style={{ color: showInfo ? "#7c7cdc" : undefined }}
          >
            {showInfo ? "✕" : "?"}
          </button>
        </div>
        ...
        {showInfo && (
          <div style={{
            ...
            <div style={{ fontSize: 9, fontWeight: 700, color: "#4a4a6a", letterSpacing: "0.1em", marginBottom: 2 }}>
              WHAT INGESTION DOES
            </div>
            {INGESTION_STEPS.map((step, i) => (
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                <span style={{
                  fontSize: 8, fontWeight: 700, color: "#7c7cdc",
                  background: "#13133a", border: "1px solid #2a2a6a",
                  borderRadius: 2, padding: "1px 4px", flexShrink: 0, marginTop: 1,
                }}>
                  {i + 1}
                </span>
                <div>
                  <div style={{ fontSize: 10, color: "#8a8ab0", letterSpacing: "0.02em" }}>{step.label}</div>
        ...
        {!showInfo && (
          <div style={{ fontSize: 10, color: "#2a2a4a", textAlign: "center", letterSpacing: "0.04em" }}>
            auto every 15 min
          </div>
        )}
      </div>
      ...
      {/* Status */}
      <div style={{ padding: "10px 16px", borderTop: "1px solid #1e1e2e", display: "flex", alignItems: "center", gap: 6, fontSize: 10, color: "#4a4a6a" }}>
