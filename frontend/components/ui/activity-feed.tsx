    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      {events?.slice(0, maxItems).map(e => (
        <div key={e.id} style={{
          ...
          padding: "5px 0", borderBottom: "1px solid #1e1e2e",
          animation: "fadeIn 0.2s ease",
        }}>
          <span style={{ color: e.color, fontSize: 12, flexShrink: 0, marginTop: 1 }}>{e.icon}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <span style={{
              fontSize: 9, color: e.color, letterSpacing: "0.06em",
              textTransform: "uppercase", marginRight: 6, fontWeight: 600,
            }}>{e.label}</span>
            <span style={{ fontSize: 11, color: "#c0c0d8", lineHeight: 1.4 }}>
              {e.message}
            </span>
          </div>
          <span style={{ fontSize: 9, color: "#4a4a6a", flexShrink: 0, marginTop: 2 }}>
            {timeAgo(e.ts)}
          </span>
        </div>
      ))}
